...
| `/pi [标识]` | 查看项目信息（省略则查看当前项目） |

切换项目后，所有 AI 对话的工具调用将在对应目录下执行，实现真正的多项目隔离。
...
```
/pa ~/code/my-app             # 添加项目
/pa ~/code/my-app myapp 我的应用  # 添加并指定标识和显示名
/pc ~/code/new-project        # 创建新目录并添加
/pl                           # 查看所有项目
/ps myapp                     # 切换到 myapp 项目
/pi                           # 查看当前项目信息
/prm myapp                    # 移除项目（目录保留）
```
...
│   ├── adapters/              # CLI 适配器
│   │   ├── base.py            # 适配器基类
│   │   ├── opencode.py        # OpenCode 适配器（单 server + directory query 参数 CWD 隔离）
│   │   ├── claudecode.py      # Claude Code 适配器
│   │   └── codex.py           # Codex 适配器
│   ├── feishu/                # 飞书模块
│   │   ├── client.py          # WebSocket 客户端
│   │   ├── api.py             # API 封装（ReplyMessageRequest 引用支持）
│   │   ├── handler.py         # 消息处理器（集成项目管理）
...
│   │   └── formatter.py       # 消息格式化
│   ├── project/               # 项目管理
│   │   ├── models.py          # Project / ProjectsConfig 数据模型
│   │   └── manager.py         # ProjectManager（增删改查、持久化）
│   ├── session/               # 会话管理
│   │   └── manager.py         # 会话管理器
│   ├── tui_commands/          # TUI 斜杠命令
│   │   ├── base.py            # 命令基类 / TUIResult
│   │   ├── opencode.py        # OpenCode 专属命令实现
│   │   ├── project.py         # 项目管理命令（/pa /pc /pl /ps /prm /pi）
│   │   ├── interactive.py     # 交互式消息追踪
│   │   └── __init__.py        # 命令路由器
│   ├── config.py              # 配置管理
│   └── main.py                # 主程序入口
├── doc/
...
- ✅ 切换项目后 AI 工具在对应目录执行（多项目真正隔离）
- ✅ 修复 OpenCode 工作目录不隔离问题（`directory` query 参数）
- ✅ 简化 OpenCode 架构（单 server + 按目录缓存 session）

### v0.0.4 (2026-03-21)

- ✅ 图片输入支持（发送图片消息，模型视觉识别分析）
- ✅ 文件输入支持（发送文档/代码文件，模型直接读取分析）
- ✅ 富文本嵌入图片支持（post 消息中的 img tag）
- ✅ 切换为 `prompt_async` 端点（与 SSE 事件流架构对齐）
- ✅ 本地临时文件自动清理（24h 过期自动删除）

### v0.0.3 (2026-03-20)

- ✅ 卡片样式美化（自动 emoji 分类图标、紧凑 Footer、优化思考面板）
- ✅ 参考 OpenClaw 飞书插件样式改进消息卡片显示效果
