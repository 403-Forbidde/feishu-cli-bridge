...
class FeishuClient:
    ...
    def __init__(self, app_id: str, app_secret: str, encrypt_key: str = "", verification_token: str = ""):
        ...
        self._message_handler: Optional[Callable[[FeishuMessage], Awaitable[None]]] = None
        self._running = False
    ...
    def _on_message_received(self, event: P2ImMessageReceiveV1) -> None:
        ...
        """
        try:
            logger.info(f"📝 收到原始消息事件: event_id={event.header.event_id if event.header else 'unknown'}")
            ...
            logger.info(f"✅ 消息解析成功 from {message.sender_id}: {message.content[:50]}...")
    ...
    def _event_to_dict(self, event: P2ImMessageReceiveV1) -> Dict[str, Any]:
        ...
        return {
            ...
            "event": {
                "sender": {
                    "sender_id": {
                        "open_id": event.event.sender.sender_id.open_id if event.event.sender.sender_id else ""
                    },
                    "sender_type": event.event.sender.sender_type if event.event.sender else "user"
                },
                "message": {
                    ...
                    "message_type": event.event.message.message_type if event.event.message else "",
